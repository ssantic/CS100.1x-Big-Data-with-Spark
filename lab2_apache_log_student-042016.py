...
dayHostCount = dayGroupedHosts.map(lambda (x,y): (x, len(list(set(y)))))
...
avgDailyReqPerHost = (sortedByDay
                      .map(lambda (x,y): (x, len(y) / len(list(set(y)))))
                      .cache())
...
badUniqueEndpoints = badEndpoints.reduceByKey(lambda x,y : x+y)
