...
def wordCount(wordListRDD):
    ...
    """
    wordsMapped = wordListRDD.map(lambda x: (x, 1))
    wordsReduced = wordsMapped.reduceByKey(lambda x,y: x+y)
    return wordsReduced
...
def removePunctuation(text):
    ...
    """
    punct = '!"#$%&\'()*+,-./:;<=>?@[\\]^_`{|}~'
    regex = re.compile('[%s]' % punct)
    
    punct_stripped = regex.sub('', text)
    whitespace_stripped = punct_stripped.strip()
    lowercased = whitespace_stripped.lower()
...
shakespeareWordsRDD = shakespeareRDD.flatMap(lambda x: split(x, ' '))
