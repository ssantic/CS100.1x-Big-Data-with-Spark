...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    """
    number_of_ratings = len(IDandRatingsTuple[1])
    average_rating = (sum(IDandRatingsTuple[1])) / float(number_of_ratings)
    
    return (IDandRatingsTuple[0],(number_of_ratings, average_rating))
...
def computeError(predictedRDD, actualRDD):
    ...
    squaredErrorsRDD = (predictedReformattedRDD
                        ...
                        .map(lambda x: float((x[1][0] - x[1][1])**2)))
...
myUnratedMoviesRDD = (moviesRDD
                      .filter(lambda x: x[0] not in myRatedMovies)
                      .map(lambda x: (0, x[0])))
